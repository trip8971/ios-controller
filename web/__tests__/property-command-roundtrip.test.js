 ...
 * volume_up, volume_down), serializing it to a JSON string via JSON.stringify
 * and then deserializing it back via JSON.parse should produce a semantically
 * equivalent Control_Command object.
 */
...
describe('Feature: ios-remote-control, Property 5: Control_Command 往返一致性', () => {
  it('any valid Control_Command should survive JSON.parse(JSON.stringify(command)) roundtrip with semantic equivalence', () => {
    fc.assert(
      fc.property(
        controlCommandArb,
        (command) => {
          // Serialize to JSON string and deserialize back
          const deserialized = JSON.parse(JSON.stringify(command));
